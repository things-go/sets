...
// Uint64 is a set of uint64s, implemented via map[uint64]struct{} for minimal memory consumption.
...
// NewUint64From creates a Uint64 from a keys of a map[uint64](? extends interface{}).
// If the value passed in is not actually a map, this will panic.
func NewUint64From(theMap interface{}) Uint64 {
	v := reflect.ValueOf(theMap)
	ret := Uint64{}

	for _, keyValue := range v.MapKeys() {
		ret[keyValue.Interface().(uint64)] = struct{}{}
	}
	return ret
}
...
func (s Uint64) ContainsAll(items ...uint64) bool {
	for _, item := range items {
		if !s.Contains(item) {
			return false
		}
...
func (s Uint64) ContainsAny(items ...uint64) bool {
	for _, item := range items {
		if s.Contains(item) {
			return true
		}
...
func (s Uint64) Merge(s2 Uint64) Uint64 {
	for key := range s2 {
		s[key] = struct{}{}
	}
	return s
...
func (s Uint64) IsSubset(s2 Uint64) bool {
	for item := range s {
		if !s2.Contains(item) {
			return false
		}
...
func (s Uint64) Each(f func(item uint64) bool) {
	for item := range s {
		if !f(item) {
			break
		}
