...
func TestUint8Set(t *testing.T) {
	...
	// improve cover
	s2 = NewUint8From(map[uint8]interface{}{1: "1", 2: "2", 3: "3"})
	s2.UnsortedList()
}
...
func TestUint8Merge(t *testing.T) {
	tests := []struct {
		s1       Uint8
		s2       Uint8
		expected Uint8
...
func TestUnt8_Each(t *testing.T) {
	expect := NewUint8(1, 2, 3, 4)
	s1 := NewUint8(1, 2, 3, 4)
	s1.Each(func(item interface{}) bool {
		require.True(t, expect.Contains(item.(uint8)))
		return item.(uint8) != 3
	})
}
